...
test.describe("SCEN-05 — manage_brigadista_assignments vs manage_assignments", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    const { json: surveysJson } = await apiGet(
      request,
    ...
    const published = (
      surveys as { title: string; id: number; status?: string }[]
    ).find((s) => s.title.startsWith("QA") && s.status === "published");
    surveyId = published?.id;

    // Find user IDs
    const { json: usersJson } = await apiGet(request, "/users", adminToken, {
      limit: "100",
    });
  ...
  test("API: assign brigadista → 200 or 201 (manage_brigadista_assignments present)", async ({
    request,
  }) => {
    test.skip(
      !surveyId || !brigadistaAlphaId,
      "Missing surveyId or brigadistaAlphaId",
    );
  ...
  test("API: assign encargado → 403 (manage_assignments absent)", async ({
    request,
  }) => {
    test.skip(
      !surveyId || !encargadoAlphaId,
      "Missing surveyId or encargadoAlphaId",
    );
  ...
  test("API: DELETE /assignments/{id} → 403 (no manage_assignments)", async ({
    request,
  }) => {
    // Pick any existing assignment — attempt to delete it must be 403
    const { json: groups } = await apiGet(
      request,
      "/assignments/groups",
      token,
      { limit: "5" },
    ...
    const { json: assigns } = await apiGet(
      request,
      `/assignments/groups/${firstGroupId}`,
      token,
    );
    const assignments =
      (assigns as Record<string, { id: number }[]>)?.assignments ?? [];
