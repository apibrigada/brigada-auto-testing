...
test.describe("SCEN-02 — Survey Creator: create ✓ edit ✓ publish ✗", () => {
  ...
  test("API: POST /admin/surveys → 201 (has create_survey)", async ({ request }) => {
  ...
  test("API: POST .../publish → 403 (missing publish_survey)", async ({ request }) => {
    test.skip(!createdSurveyId, "Needs survey from previous test");
    // Get version_id first
    const { json: detail } = await apiGet(request, `/admin/surveys/${createdSurveyId}`, token);
    const versionId = ((detail as Record<string, unknown>)?.current_version as { id?: number })?.id
      ?? ((detail as Record<string, unknown[]>)?.versions as { id: number }[])?.[0]?.id;
    if (!versionId) {
  ...
  test("CMS UI: Publish button is absent/disabled on survey detail page", async ({ page }) => {
    ...
    await page.locator("table tbody tr, [data-testid='survey-row']").first().click();
    await page.waitForLoadState("networkidle");
    ...
    expect(isAbsent || isDisabled, "Publish button must be absent or disabled for SCEN-02").toBe(true);
  });
...
test.describe("SCEN-03 — Survey Publisher: publish ✓ create ✗ edit ✗", () => {
  ...
  test("API: POST /admin/surveys → 403 (missing create_survey)", async ({ request }) => {
  ...
  test("API: GET /admin/surveys → 200 (has view_surveys)", async ({ request }) => {
...
test.describe("SCEN-06 — Scoped Encargado: only 2 surveys accessible", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    const items = Array.isArray(json) ? json : ((json as Record<string, unknown>)?.items ?? []) as unknown[];
    ...
    if (adminToken) {
      const { json: allJson } = await apiGet(request, "/admin/surveys/metadata", adminToken, { limit: "20" });
      ...
      outOfScopeId = (allItems as { id?: number }[]).find((s) => !scopedIds.has(s.id))?.id;
    }
  });

  test("API: GET /admin/surveys/metadata returns exactly 2 surveys", async ({ request }) => {
    const { status, json } = await apiGet(request, "/admin/surveys/metadata", token);
    expect(status).toBe(200);
    const items = Array.isArray(json) ? json : ((json as Record<string, unknown>)?.items ?? []) as unknown[];
    expect(items.length, "SCEN-06: scoped user must see exactly 2 surveys").toBe(2);
  });

  test("API: in-scope survey detail → 200", async ({ request }) => {
    test.skip(!inScopeId, "No in-scope survey ID resolved");
    const { status } = await apiGet(request, `/admin/surveys/${inScopeId}`, token);
    expect(status).toBe(200);
  });

  test("API: out-of-scope survey detail → 403 or 404", async ({ request }) => {
    test.skip(!outOfScopeId, "No out-of-scope survey ID resolved");
    const { status } = await apiGet(request, `/admin/surveys/${outOfScopeId}`, token);
    expect([403, 404], `Expected 403 or 404 for out-of-scope survey, got ${status}`).toContain(status);
  });

  test("API: responses for out-of-scope survey → 403 or 404", async ({ request }) => {
    test.skip(!outOfScopeId, "No out-of-scope survey ID resolved");
    const { status } = await apiGet(request, `/admin/responses`, token, { survey_id: outOfScopeId! });
    expect([403, 404], `Expected 403 or 404 for out-of-scope responses, got ${status}`).toContain(status);
  });
