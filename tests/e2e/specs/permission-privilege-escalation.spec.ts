...
test.describe("SCEN-12 — Privilege Escalation via Wildcard Targets", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    const adminEmail =
      process.env.E2E_LOGIN_EMAIL_ROLE_1 ?? "admin@brigada.com";
  ...
  test("API: PATCH /users/{admin_principal_id} → 403 (CRITICAL: wildcard must not reach system admin)", async ({
    request,
  }) => {
  ...
  test("API: DELETE /users/{admin_principal_id} → 403 (CRITICAL: wildcard cannot delete admin)", async ({
    request,
  }) => {
    test.skip(!adminPrincipalId, "Admin principal user ID not found");

    const resp = await request.delete(
      `${API_BASE_URL}/users/${adminPrincipalId}`,
      {
  ...
  test("API: POST /roles → 403 (no manage_access_control)", async ({
    request,
  }) => {
  ...
  test("API: POST /users/reset-password/{admin_id} → 403", async ({
    request,
  }) => {
    test.skip(!adminPrincipalId, "Admin principal user ID not found");
